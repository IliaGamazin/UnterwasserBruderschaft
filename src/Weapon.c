...
void Weapon_shoot(Weapon *weapon, BulletManager *bullet_manager, Vector2 origin, Vector2 direction) {
    ...
    for (size_t i = 0; i < weapon->bullets_num; i++) {
        double randomAngle = (rand() / (double)RAND_MAX) * 6 - 3; // Random angle in range [-3; 3] degrees
        Vector2 rotatedDirection = Vector2_rotated(direction, degrees_to_radians(randomAngle));
        BulletManager_add(bullet_manager, origin, rotatedDirection, 30, Rgba_new(255, 255, 255, 255));
    }
